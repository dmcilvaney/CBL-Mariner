...
In VSCode, there are specialzied prompts for common tasks. They can be accessed from the chat  window by typing `/azl-...` to filter for Azure Linux prompts:
...
```bash
# Fully i nteractive mode
copilot --add-dir .
```

```bash
# Run a specific prompt prompt, then drop to interactive mode
