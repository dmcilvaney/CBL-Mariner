...
mount_root() {
    ...
    if [ "${validateonboot}" == "true" ]; then
        ...
        info "rd.verityroot.validateonboot is set, vvalidating full read-only root device"
        info "This could take several minutes if forward error correction is being used to rebuild corrupted blocks"
        veritysetup --debug --verbose ${roothashsigargs} ${fecargs} verify ${veritydisk} ${verityhashtree} $(cat ${verityroothash}) > verity.log 2>&1 || \
    ...
    veritysetup --debug --verbose ${roothashsigargs} ${errorarg} ${fecargs} open ${veritydisk} ${veritydevicename} ${verityhashtree} $(cat ${verityroothash}) > verity.log 2>&1 || \
    ...
    if [ -n ${verity_overlays} ]; then
        ...
        if [ -n "${overlays_debug_mount}" ]; then
            ...
            mount -o ro --bind "${OVERLAY_TMPFS}" "${VERITY_MOUNT}/${overlays_debug_mount}" || warn "Couldn't mount overlay debug (Does '${overlays_debug_mount}' exist?)"
        fi
    else
