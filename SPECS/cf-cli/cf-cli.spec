...
%check
# We want to install the correct version of ginkgo into the environment
export GINKOVERSION="$(go list -m github.com/onsi/ginkgo | cut -d' ' -f2)"
export PATH=$PATH:$(go env GOPATH)/bin
(cd .. && go install github.com/onsi/ginkgo/ginkgo@$GINKOVERSION)

# NOTE: The unit tests have known failures with SSL, needs investigation.
# NOTE: We would also like to run `units-full` if possible, once failures are resolved.
make units && \
./out/cf --version
...
- Enhance check section to run unit tests.
