...
| `release.calculation` | Release tag handling (`"auto"`, `"autorelease"`, or `"manual"`) | `"manual"` |
...
By default (`release.calculation = "auto"`), `azldev` auto-calculates the `Release` tag during rendering. There are three modes:
