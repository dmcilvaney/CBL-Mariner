...
1. Check if `${input:component_name}` already exists: `azldev comp query -p ${input:component_name}`
2. If it doesn't exist, add a bare inline entry to inspect the upstream spec first
3. Use `azldev comp prep-sources -p ${input:component_name} --skip-overlays -o <dir>` to pull the upstream spec
...
7. Validate: `azldev comp prep-sources -p ${input:component_name} -o <dir>` (with overlays) and diff against the skip-overlays output
8. Build: `azldev comp build -p ${input:component_name}`
