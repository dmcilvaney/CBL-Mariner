	...
	for {
		...
		if !stopBuilding {
			if res.Err == nil {
				...
				logger.Log.Errorf("DELTA: Build result: %v", res)

				if res.Node.Type == pkggraph.TypeBuild && res.WasDelta {
					logger.Log.Tracef("DELTA: This is a delta result, update the graph with the new delta files.")
					logger.Log.Tracef("DELTA: %v", res.Node)
...
func setAssociatedDeltaPaths(res *schedulerutils.BuildResult, builtFiles []string, pkgGraph *pkggraph.PkgGraph, graphMutex *sync.RWMutex) (err error) {
	...
	for _, builtFile := range builtFiles {
		...
		logger.Log.Tracef("DELTA: Built delta file: %s", builtFile)
		builtFileMap[filepath.Base(builtFile)] = builtFile
	}

	logger.Log.Errorf("buidl file map: %v", builtFileMap)
	...
	for _, node := range pkgGraph.AllRunNodes() {
		...
		if ok {
			...
			if node.State == pkggraph.StateDelta {
				logger.Log.Tracef("DELTA: Found delta run node: %s", node)
				// Update the node to point at the actual RPM path from our map of built files
				logger.Log.Tracef("DELTA: Updating delta run node from %s to %s", node.RpmPath, builtFile)
				node.RpmPath = builtFile
			} else {
				logger.Log.Errorf("unexpected node type/state when scanning for run nodes: %s", node)
			}
		}
	}
