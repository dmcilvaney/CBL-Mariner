...
func TestShouldFailParsingBothVerityAndEncryption_SystemConfig(t *testing.T) {
	...
	assert.Error(t, err)
	assert.Equal(t, "invalid [ReadOnlyVerityRoot] & [Encryption]: verity root currently does not support root encryption", err.Error())

	err = remarshalJSON(badBothEncryptionVerityConfig, &checkedSystemConfig)
	assert.Error(t, err)
	assert.Equal(t, "failed to parse [SystemConfig]: invalid [ReadOnlyVerityRoot] & [Encryption]: verity root currently does not support root encryption", err.Error())
}
...
func TestShouldFailParsingVerityRootWithNoBoot_SystemConfig(t *testing.T) {
	...
	err := badPartitionSettingsConfig.IsValid()
	assert.Error(t, err)
	assert.Equal(t, "invalid [ReadOnlyVerityRoot] or [Encryption]: must have a separate partition mounted at '/boot'", err.Error())

	err = remarshalJSON(badPartitionSettingsConfig, &checkedSystemConfig)
	assert.Error(t, err)
	assert.Equal(t, "failed to parse [SystemConfig]: invalid [ReadOnlyVerityRoot] or [Encryption]: must have a separate partition mounted at '/boot'", err.Error())
}
