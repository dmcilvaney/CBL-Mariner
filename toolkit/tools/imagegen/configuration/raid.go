...
// 10: Mirror on four+ disks with striping
...
func (r *RaidConfig) IsEmpty() bool {
	...
	return len(r.ComponentPartIDs) == 0
}
...
func (r *RaidConfig) IsValid() (err error) {
	...
	if len(r.RaidID) != 0 && len(r.ComponentPartIDs) == 0 {
		return fmt.Errorf("invalid [RaidConfig]: Raid '%s' must have non-empty ComponentPartIDs", r.RaidID)
	}
