...
//   initramfs
// - Custom name for the mounted root (default is "verity_root_fs")
...
//   system keyring. The signature file should be called "rootfs.p7" and must be stored in
...
// - VerityErrorBehavior: System behavior when encountering an unrecoverable verity corruption
// - TempOverlays: Mount these paths as writable overlays backed by a ram backed tmpfs
//   Overlays should not overlap each other. If a directory is not already present it
//   will be created automatically. Persistant overlays can be created by mounting
...
const (
	// Default value used for Android's dm-verity FEC, gives 16MiB recovery for a 2GiB disk with 0.8% overhead
...
// IsValid returns an error if the GeneralVerityMount is not valid
func (v *ReadOnlyVerityRoot) IsValid() (err error) {
	// The default disabled config doesn't not generate a default name, only check if we have explicitly
	...
	for i, overlayA := range v.TmpfsOverlays {
		for j, overlayB := range v.TmpfsOverlays {
			if i == j {
				// Skip checking a path against itself
				continue
			}
