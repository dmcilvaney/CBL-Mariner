...
func TestShouldFailParsingInvalidPartitionFlag_PartitionFlag(t *testing.T) {
	...
	assert.Error(t, err)
	assert.Equal(t, "failed to parse [PartitionFlag]: invalid value for Flag (not_a_partition_flag)", err.Error())
}
...
func TestShouldFailParsingInvalidJSON_PartitionFlag(t *testing.T) {
	...
	assert.Error(t, err)
	assert.Equal(t, "failed to parse [PartitionFlag]: json: cannot unmarshal number into Go value of type configuration.IntermediateTypePartitionFlag", err.Error())
}
