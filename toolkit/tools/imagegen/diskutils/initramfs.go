...
// CreateInitramfs creates a new initramfs with
...
// OpenInitramfs modifies an existing initramfs
func OpenInitramfs(initramfsPath string) (initramfs InitramfsMount, err error) {
	...
	//cpio.Reader has no Close() function
	...
	for {
		...
		logger.Log.Infof("File %s caused %d bytes to be transfered to new archive", nextFileHeader.Name, bytesIO)
		logger.Log.Infof("Buffer unread length: %d", initramfs.outputBuffer.Len())
	}
...
// Close closes all initramfs mount resources
func (i *InitramfsMount) Close() (err error) {
	var bytesIO int

	logger.Log.Debug("Closing initramfs file")
...
func (i *InitramfsMount) AddFileToInitramfs(sourcePath, destPath string) (err error) {
	...
	if !fileInfo.Mode().IsRegular() {
		...
		logger.Log.Infof("Added special file %s", header.Name)
		return
	}
	...
	logger.Log.Infof("New file %s caused %d bytes to be transfered to new archive", header.Name, bytesIO)
