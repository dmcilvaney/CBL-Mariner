...
func main() {
	...
	if err != nil {
		logger.Log.Fatalf("Failed to read graph to file. Error: %s", err)
	}
	...
	if *tryDownloadDeltaRPMs || hasUnresolvedNodes {
		...
		if err != nil {
			logger.Log.Fatalf("failed to setup new cloner: %s", err)
		}
		defer cloner.Close()
	}
	...
	if hasUnresolvedNodes {
		...
		if err != nil {
			logger.Log.Fatalf("Failed to resolve graph. Error: %s", err)
		}
	} else {
	...
	if err != nil {
		logger.Log.Fatalf("Failed to write cache graph to file. Error: %s", err)
	}

	// Optional delta build cache hydration, panics on error
	if *tryDownloadDeltaRPMs {
		err = downloadDeltaNodes(dependencyGraph, cloner)
		if err != nil {
			logger.Log.Fatalf("Failed to download delta RPMs. Error: %s", err)
		}
	...
	if *tryDownloadDeltaRPMs || hasUnresolvedNodes {
		...
		if err != nil {
			logger.Log.Fatalf("Failed to convert downloaded RPMs into a repo. Error: %s", err)
		}
	}
}
...
func downloadDeltaNodes(dependencyGraph *pkggraph.PkgGraph, cloner *rpmrepocloner.RpmRepoCloner) (err error) {
	...
	isGraphOptimized, deltaPkgGraphCopy, _, err := schedulerutils.InitializeGraph(*outputGraph, packageVersToBuild, deltaBuildGraph, useSRPMPathMatching)
	if err != nil {
		logger.Log.Fatalf("Failed to initialize graph for delta package downloading. Error: %s", err)
		return
	}
...
func resolveGraphNodes(dependencyGraph *pkggraph.PkgGraph, inputSummaryFile, outputSummaryFile string, toolchainPackages []string, cloner *rpmrepocloner.RpmRepoCloner, stopOnFailure bool) (err error) {
	...
	if strings.TrimSpace(inputSummaryFile) == "" {
		...
		for _, n := range dependencyGraph.AllRunNodes() {
			if n.State == pkggraph.StateUnresolved {
				resolveErr := resolveSingleNode(cloner, n, true, toolchainPackages, fetchedPackages, prebuiltPackages, *outDir)
...
func downloadAllAvailableDeltaRPMs(realDependencyGraph, dependencyGraphDeltaCopy *pkggraph.PkgGraph, cloner *rpmrepocloner.RpmRepoCloner, stopOnFailure bool) (err error) {
	...
	logger.Log.Errorf("Resolving build nodes")
	for _, n := range dependencyGraphDeltaCopy.AllBuildNodes() {
		// Implicit ndoes cause us troubles since we don't know exactly which RPMs they will build (so the cache fetcher
		...
		logger.Log.Errorf("Resolving build node %s", n)
	...
	for _, n := range skippedNodes {
		logger.Log.Errorf("Fixing up skipped node %s", n)
...
//   - temporaryBuildNode: The build node to update, this node need not be in the actual graph and will be used as a reference
...
func downloadSingleDeltaRPM(realPkgGraph *pkggraph.PkgGraph, temporaryBuildNode *pkggraph.PkgNode, cloner *rpmrepocloner.RpmRepoCloner, deltaRpmDir string, pathMap *map[string]string) (foundMatch bool, err error) {
	...
	if temporaryBuildNode.Type != pkggraph.TypeBuild {
		err = fmt.Errorf("node '%s' is not a build node, can't download delta RPM", temporaryBuildNode)
		return false, err
	}
	lookup, err = realPkgGraph.FindExactPkgNodeFromPkg(temporaryBuildNode.VersionedPkg)
	if err != nil {
		err = fmt.Errorf("can't find build node '%s' in graph: %w", temporaryBuildNode, err)
		return false, err
	}
	if lookup == nil || lookup.BuildNode == nil || lookup.RunNode == nil {
		err = fmt.Errorf("can't find build and run lookup '%v' in graph", lookup)
		return false, err
	}
	...
	// Get the final path for the build node if we don't convert it to a delta node
	dstPath := realRunNode.RpmPath
	foundFinalRPM, _ := file.IsFile(dstPath)

	// Only download dependencies for delta RPMs if we don't already have the RPM in the out/RPMS folder
	if !foundFinalRPM {
		resolveErr := resolveSingleNode(cloner, temporaryBuildNode, downloadDependencies, nil, make(map[string]bool), make(map[string]bool), deltaRpmDir)
		// Failing to clone a dependency should not halt a build.
		// The build should continue and attempt best effort to build as many packages as possible.
		if resolveErr != nil {
			logger.Log.Infof("Can't find delta RPM to download for %s: %s", temporaryBuildNode, resolveErr)
			return false, nil
		}
		logger.Log.Tracef("Updating real node '%s' with info from newly cached delta node '%s'", realBuildNode, temporaryBuildNode)

		// Check that the RPM we are getting matches the expected out/RPM path using the base name of the file path
		cachedRPMPath := temporaryBuildNode.RpmPath
		if filepath.Base(cachedRPMPath) != filepath.Base(dstPath) {
			err = fmt.Errorf("cached RPM '%s' does not match expected RPM '%s'", filepath.Base(cachedRPMPath), filepath.Base(dstPath))
			return false, err
		}
		...
		(*pathMap)[realBuildNode.RpmPath] = temporaryBuildNode.RpmPath
		realBuildNode.RpmPath = temporaryBuildNode.RpmPath
		realRunNode.RpmPath = temporaryBuildNode.RpmPath

		logger.Log.Errorf("PathMap updated: %v", *pathMap)
		...
		logger.Log.Tracef("Found out RPM for '%s' at '%s', skipping delta download.", temporaryBuildNode, dstPath)
	}
...
// also be updated to point to the new RPM since the scheduler uses the run node to find the RPM to install.
//   - pkgGraph: The graph to update
//   - temporaryImplicitNode: The implicit build node to update, this node need not be in the actual graph and will be used as a reference
//     to find the actual build node in the graph.
//   - pathMap: The map of original RPM paths to delta RPM paths that we have already downloaded.
func fixupDeltaImplicitNode(realPkgGraph *pkggraph.PkgGraph, temporaryImplicitNode *pkggraph.PkgNode, pathMap *map[string]string) (err error) {
	var lookup *pkggraph.LookupNode

	logger.Log.Errorf("Implicit node '%s' is a delta RPM, fixing up build and run nodes.", temporaryImplicitNode)

	lookup, err = realPkgGraph.FindExactPkgNodeFromPkg(temporaryImplicitNode.VersionedPkg)
	if err != nil {
		err = fmt.Errorf("can't find implicit lookup node '%s' in graph: %w", temporaryImplicitNode, err)
		return err
	}
	if lookup == nil || lookup.BuildNode == nil || lookup.RunNode == nil {
	...
	if deltaPath, ok := (*pathMap)[dstPath]; ok {
		logger.Log.Tracef("Found delta RPM for '%s' at '%s', updating build and run nodes.", temporaryImplicitNode, dstPath)
		realBuildNode.State = pkggraph.StateDelta
		realRunNode.State = pkggraph.StateDelta
		...
		logger.Log.Tracef("Can't find delta RPM for '%s' at '%s', skipping implici delta update.", temporaryImplicitNode, dstPath)
		return
	}
...
func resolveSingleNode(cloner *rpmrepocloner.RpmRepoCloner, node *pkggraph.PkgNode, cloneDeps bool, toolchainPackages []string, fetchedPackages, prebuiltPackages map[string]bool, outDir string) (err error) {
	...
	// Try to pre-cache all the packages in one go if there are multiple options. This should speed up the subsequent individual checks.
	if len(resolvedPackages) > 1 {
		logger.Log.Warnf("HI")
		desiredPackages := make([]*pkgjson.PackageVer, 0, len(resolvedPackages))
		for _, resolvedPackage := range resolvedPackages {
			if !fetchedPackages[resolvedPackage] {
				desiredPackages = append(desiredPackages, &pkgjson.PackageVer{
					Name: resolvedPackage,
				})
			}
		}
		// This is best effort so we don't care if it fails.
		_, _ = cloner.Clone(cloneDeps, desiredPackages...)
	}
...
func assignRPMPath(node *pkggraph.PkgNode, outDir string, resolvedPackages []string) (err error) {
	...
	if len(rpmPaths) > 1 {
		...
		logger.Log.Errorf("picking %v", resolvedRPMs[0])
